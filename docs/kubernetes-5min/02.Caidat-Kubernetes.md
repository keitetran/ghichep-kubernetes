...
- Trong đó:
  - `192.168.1.25`: là IP của node k8s-master.
  - `--apiserver-advertise-address`: là địa chỉ của node k8s-master, địa chỉ này cần truyền thông được với các node còn lại trong cụm cluster. Trong ví dụ này node k8s-master có địa chỉ là: 192.168.1.25.
...
- Kết quả của lệnh trên như bên dưới hoặc như ảnh http://prntscr.com/i5icxv
  ...
  ```sh
  root@k8s-master:~#   kubeadm init --apiserver-advertise-address 192.168.1.25 --pod-network-cidr=10.244.0.0/16
  ...
  [certificates] apiserver serving cert is signed for DNS names [k8s-master kubernetes kubernetes.default kubernetes.default.svc kubernetes.default.svc.cluster.local] and IPs [10.96.0.1 192.168.1.25]
  ...
  as root:
    ...
    kubeadm join --token 1aeb1d.89a15bf6272c8274 192.168.1.25:6443 --discovery-token-ca-cert-hash sha256:cb8e0cd1238dc8fe8b1b2f16fe02817425005f04a8ddd09a7c19db08b75f72eb
...
- Từ bản 1.9 trở lên, thực hiện lệnh dưới để tạo token trên node k8s-master, kết quả trả vê được sử dụng để thực hiện trên các k8s-node1 và k8s-node2
  ...
  - Kết quả như bên dưới, lưu giá trị trong cột token để sử dụng cho việc join các k8s-node1 và k8s-node2 vào cụm cluster:
    ...
    ```sh
    kubeadm join --token 150984.0da1fe160e5113f0 192.168.1.25:6443 --discovery-token-ca-cert-hash sha256:cb8e0cd1238dc8fe8b1b2f16fe02817425005f04a8ddd09a7c19db08b75f72eb
    ```
...
- Đứng trên máy Laptop hoặc máy khác cùng dải mạng với dải IP của các node trong cụ K8S, mở trình duyệt web và truy cập với địa chỉ: `http://192.168.1.25:32136` hoặc `http://192.168.1.26:32136` hoặc `http://192.168.1.27:32136`, chúng ta sẽ thấy kết quả như ảnh:  http://prntscr.com/jhiy1r hoặc http://prntscr.com/jhiy3x
...
- Kiểm tra thêm bằng các lệnh
	...
	- Sử dụng port ở kết quả để truy cập, với node port sẽ là `http://192.168.1.25:32316/` hoặc `http://192.168.1.26:32316/` hoặc `http://192.168.1.27:32316/`
