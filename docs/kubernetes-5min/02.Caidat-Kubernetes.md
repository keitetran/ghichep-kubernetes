...
### 1. Môi trường
- Ubuntu 16.04 - 64 bit (3 node: 01 Node unbutu-server-18, 02 node k8s-node1 & k8s-node2)
...
#### 3.1. Đặt hostname và ip cho node `unbutu-server-18`
...
- Đặt ip cho node `unbutu-server-18` bằng cách chạy lệnh dưới để sửa file file `/etc/network/interfaces`
...
- Đặt hostname cho máy `unbutu-server-18` bằng cách sửa nội dung các file `/etc/hosts` và `/etc/hostname`
  - Chạy lệnh dưới để khai báo hostname cho `k8s-node1`
    ```sh
    cat << EOF > /etc/hosts
    127.0.0.1       localhost unbutu-server-18
    192.168.1.26       unbutu-server-18
    192.168.1.26       k8s-k8s-node1
    192.168.1.27       k8s-node2
    EOF
    ```
    
  - File `/etc/hostname` được sửa bằng lệnh dưới.
    ```sh
    echo unbutu-server-18 > /etc/hostname
    ```
    
- Khởi động lại node unbutu-server-18
  ```sh
  init 6
  ```
...
- Đứng trên node `unbutu-server-18` thực hiện lệnh dưới để thiết lập cluster

  ```sh
  kubeadm init --apiserver-advertise-address 192.168.1.26 --pod-network-cidr=10.244.0.0/16
  ```
  
- Trong đó:
  - `192.168.1.26`: là IP của node unbutu-server-18.
  - `--apiserver-advertise-address`: là địa chỉ của node unbutu-server-18, địa chỉ này cần truyền thông được với các node còn lại trong cụm cluster. Trong ví dụ này node unbutu-server-18 có địa chỉ là: 192.168.1.26.
...
- Kết quả của lệnh trên như bên dưới hoặc như ảnh http://prntscr.com/i5icxv
  ...
  ```sh
  root@unbutu-server-18:~#   kubeadm init --apiserver-advertise-address 192.168.1.26 --pod-network-cidr=10.244.0.0/16
  ...
  [certificates] apiserver serving cert is signed for DNS names [unbutu-server-18 kubernetes kubernetes.default kubernetes.default.svc kubernetes.default.svc.cluster.local] and IPs [10.96.0.1 192.168.1.26]
  ...
  [markunbutu-server-18] Will mark node unbutu-server-18 as unbutu-server-18 by adding a label and a taint
  [markunbutu-server-18] unbutu-server-18 unbutu-server-18 tainted and labelled with key/value: node-role.kubernetes.io/unbutu-server-18=""
  ...
  Your Kubernetes unbutu-server-18 has initialized successfully!
  ...
  as root:
    ...
    kubeadm join --token 1aeb1d.89a15bf6272c8274 192.168.1.26:6443 --discovery-token-ca-cert-hash sha256:cb8e0cd1238dc8fe8b1b2f16fe02817425005f04a8ddd09a7c19db08b75f72eb

  root@unbutu-server-18:~#
  ```

- Quan sát của sổ ssh và thực hiện theo thông báo, thực hiện tiếp trên node `unbutu-server-18` để cấu hình `kubectl` cho node unbutu-server-18.
...
- Đứng trên node unbutu-server-18 cài đặt Pod network.
- K8S có nhiều lựa chọn cho giải pháp network để kết nối các container, trong hướng dẫn này chúng tôi sử dụng `flannel`
  ...
  - Kết quả của lệnh trên như sau:
	...
	```sh
	root@unbutu-server-18:~# kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml
...
- Từ bản 1.9 trở lên, thực hiện lệnh dưới để tạo token trên node unbutu-server-18, kết quả trả vê được sử dụng để thực hiện trên các k8s-node1 và k8s-node2
  ...
  - Kết quả như bên dưới, lưu giá trị trong cột token để sử dụng cho việc join các k8s-node1 và k8s-node2 vào cụm cluster:
    ...
    ```sh
    kubeadm join --token 150984.0da1fe160e5113f0 192.168.1.26:6443 --discovery-token-ca-cert-hash sha256:cb8e0cd1238dc8fe8b1b2f16fe02817425005f04a8ddd09a7c19db08b75f72eb
    ```
...
- Kết quả trả về là:
  ...
  [preflight] Running pre-flight checks.
          [WARNING FileExisting-crictl]: crictl not found in system path
  [discovery] Trying to connect to API Server "192.168.1.26:6443"
  [discovery] Created cluster-info discovery client, requesting info from "https://192.168.1.26:6443"
  [discovery] Requesting info from "https://192.168.1.26:6443" again to validate TLS against the pinned public key
  [discovery] Cluster info signature and contents are valid and TLS certificate validates against pinned roots, will use API Server "192.168.1.26:6443"
  [discovery] Successfully established connection with API Server "192.168.1.26:6443"

  This node has joined the cluster:
  * Certificate signing request was sent to unbutu-server-18 and a response
    was received.
  * The Kubelet was informed of the new secure connection details.

  Run 'kubectl get nodes' on the unbutu-server-18 to see this node join the cluster.
  root@k8s-node1:~#
  ```

- `Lưu ý`: là cần thực hiện trên cả 02 node `k8s-node1` và `k8s-node2`

- Sau khi thực hiện join cả 02 `k8s-node1` và `k8s-node2` thì quay lại node `unbutu-server-18` để kiểm tra các node xem đã join được hay chưa.
...
- Kết quả trả về của lệnh trên như sau.
  ...
  unbutu-server-18    Ready     unbutu-server-18    9h        v1.9.2
...
- Kết quả như bên dưới là ok (kiểm tra cột `STATUS`). 
  ...
  kube-system   etcd-unbutu-server-18                      1/1       Running   0          9h
  kube-system   kube-apiserver-unbutu-server-18            1/1       Running   0          9h
  kube-system   kube-controller-manager-unbutu-server-18   1/1       Running   0          9h
  ...
  kube-system   kube-scheduler-unbutu-server-18            1/1       Running   0          9h
  ```
...
- Trong một vài trường hợp cột `STATUS` sẽ có trạng thái `Pending, ContainerCreating,ImagePullBackOf` đối với một số thành phần, có thể chờ hoặc kiểm tra bằng lệnh `kubectl describe pod <ten_pod> --namespace=kube-system` , ở đây tên pod được lấy từ cột `NAME`.
	...
	kubectl describe pod kube-scheduler-unbutu-server-18 --namespace=kube-system
	```
...
- Tạo 02 container với images là nginx, 2 container này chạy dự phòng cho nhau, port của các container là 80
	...
	- Kết quả như sau:
		...
		```sh
		root@unbutu-server-18:~# kubectl run test-nginx --image=nginx --replicas=2 --port=80
...
- Ta có thể kiểm tra lại các container nằm trong các POD (khái niệm POD đọc lại ở trước đó hoặc chuyển sang phần các khái niệm để đọc thêm) bằng lệnh
	...
	- Kết quả: 
		```
		root@unbutu-server-18:~# kubectl get pods -o wide
...
- Ngoài ra ta có thể sử dụng lệnh để  dưới để xem các service nào đã sẵn sàng để deployment.
	...
	- Kết quả như bên dưới (hãy quan sát cột `AVAILABLE`, giá trị này sẽ thay đổi khi ta thực hiện quá trình deployment các service này).
		...
		```sh
		root@unbutu-server-18:~# kubectl get deployment
...
- Tới bước này, chúng ta chưa thể truy cập vào các container được, cần thực hiện thêm bước deploy các container với các tùy chọn phù hợp, cụ thể như sau
	...
	- Kết quả 
		...
		```sh
		root@unbutu-server-18:~# kubectl expose deploy test-nginx --port 80 --target-port 80 --type NodePort
...
- Quan sát kỹ hơn ứng dụng web server vừa tạo ở trên bằng lệnh 
	...
	- Kết quả như bên dưới, hãy lưu ý kết quả này.
		```sh
		root@unbutu-server-18:~# kubectl describe service test-nginx
		Name:                     test-nginx
		Namespace:                default
...
- Đứng trên node unbutu-server-18 thực hiện curl vào một trong các IP sau: 
	...
	- Kết quả:
		...
		```sh
		root@unbutu-server-18:~# curl  10.244.1.5
		<!DOCTYPE html>
		<html>
		<head>
...
- Đứng trên node `unbutu-server-18` và thực hiện kiểm tra port được ánh xạ với container (trong kết quả trên là port `32136/TCP`)
  ...
  - Kết quả:	
	...
	```
	root@unbutu-server-18:~# ss -lan | grep 32136
	tcp    LISTEN     0      128      :::32136                :::*
	root@unbutu-server-18:~#
	```
			
- Đứng trên máy Laptop hoặc máy khác cùng dải mạng với dải IP của các node trong cụ K8S, mở trình duyệt web và truy cập với địa chỉ: `http://192.168.1.26:32136` hoặc `http://192.168.1.26:32136` hoặc `http://192.168.1.27:32136`, chúng ta sẽ thấy kết quả như ảnh:  http://prntscr.com/jhiy1r hoặc http://prntscr.com/jhiy3x
...
- Sử dụng các lệnh `kubectl get services` để biết được các vices được deploy với việc ánh xạ port là bao nhiêu (đây có thể là cách xem port được ánh xạ với các node).
	...
	```sh
	root@unbutu-server-18:~# kubectl get services
...
- Sau đó thực hiện lệnh dưới để deploy ứng dụng
	...
	```
	root@unbutu-server-18:~# kubectl create -f apache-app.yaml
...
- Kiểm tra thêm bằng các lệnh
	...
	- Sử dụng port ở kết quả để truy cập, với node port sẽ là `http://192.168.1.26:32316/` hoặc `http://192.168.1.26:32316/` hoặc `http://192.168.1.27:32316/`
